...
//   const label = viewLabel ?? "View Details";
...
//             href={viewHref}
//             aria-label={label}
//             className="flex items-center justify-between text-xs sm:text-sm text-gray-600 dark:text-gray-400 hover:text-[#19d21f] dark:hover:text-green-400 transition-colors"
//           >
//             <span>{label}</span>
...
//             aria-label={label}
//             className="flex w-full items-center justify-between text-xs sm:text-sm text-gray-600 dark:text-gray-400 hover:text-[#19d21f] dark:hover:text-green-400 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
//           >
//             <span>{label}</span>
//             <ChevronRight className="h-3 w-3 sm:h-4 sm:w-4" />
//           </button>
...
// export function StatCardOpposite({
//   icon,
//   title,
//   value,
//   iconGradient,
...
//             href={viewHref}
//             aria-label={label}
//             className="flex items-center justify-between text-xs sm:text-sm md:text-base text-gray-600 dark:text-gray-400 hover:text-[#19d21f] dark:hover:text-green-400 transition-colors"
//           >
//             <span>{label}</span>
...
//             disabled={!onViewDetails}
//             aria-label={label}
//             className="flex w-full items-center justify-between text-xs sm:text-sm md:text-base text-gray-600 dark:text-gray-400 hover:text-[#19d21f] dark:hover:text-green-400 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
//           >
//             <span>{label}</span>
//             <ChevronRight className="h-3 w-3 sm:h-4 sm:w-4 md:h-5 md:w-5" />
//           </button>
...
interface StatCardProps {
  ...
  title?: string;
  value?: string | number;
  color?: string; // header background override
  iconBg?: string; // deprecated, kept for compatibility
  iconGradient?: string; // gradient classes for the icon
  children?: ReactNode;
  loading?: boolean;
  onViewDetails?: () => void;
...
export function StatCardOpposite({
  ...
  if (loading) {
    return (
      <Card
        className="h-full flex flex-col justify-between overflow-hidden shadow-md transition-shadow border border-gray-100 dark:border-gray-800 dark:shadow-green-900/10 dark:bg-gray-900 animate-pulse"
        aria-busy="true"
        aria-live="polite"
      >
        <div
          className={`p-3 sm:p-4 md:p-5 ${color ? color : "bg-white dark:bg-gray-900"} flex items-center gap-2 sm:gap-3 md:gap-4`}
        >
          <div className="p-1.5 sm:p-2 md:p-3 rounded-full bg-gradient-to-r from-gray-200 to-gray-300 dark:from-gray-700 dark:to-gray-600 flex-shrink-0">
            <div className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 bg-gray-300 dark:bg-gray-600 rounded-full" />
          </div>
          <div className="flex flex-col justify-center flex-1 min-w-0 space-y-2">
            <div className="h-2.5 sm:h-3 w-20 sm:w-28 rounded bg-gray-200 dark:bg-gray-700" />
            <div className="h-5 sm:h-6 w-24 sm:w-32 rounded bg-gray-300 dark:bg-gray-600" />
          </div>
        </div>
        <div className="border-t border-gray-100 dark:border-gray-800 p-2 sm:p-3 md:p-4 bg-[#f9fdf9] dark:bg-gray-900/40">
          <div className="h-3 sm:h-4 w-20 sm:w-28 rounded bg-gray-200 dark:bg-gray-700 mx-auto" />
        </div>
      </Card>
    );
